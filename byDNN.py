...
#Y = df['out_yn'].map(lambda x: 1.0 if x== 1 else 0.0) 
# <<<<<< Y�� 1�� N�� 0���� �ٲٴ� �κ�
...
print(Y_train[0:100, 0:2])
print(Y_test[0:100, 0:2])
...
W1 = tf.get_variable("W1", shape=[29, 512],
                     initializer=tf.contrib.layers.xavier_initializer())
b1 = tf.Variable(tf.random_normal([512]))
L1 = tf.nn.relu(tf.matmul(X, W1) + b1)
L1 = tf.nn.dropout(L1, keep_prob=keep_prob)

W2 = tf.get_variable("W2", shape=[512, 1024],
                     initializer=tf.contrib.layers.xavier_initializer())
...
for epoch in range(training_epochs):
    ...
    while True:
        try:
            batch_xs, batch_ys =  sess.run([f, l])
